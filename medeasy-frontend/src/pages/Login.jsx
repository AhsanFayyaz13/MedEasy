            ...
            <Card className="auth-card">
              <Card.Body className="p-4 p-md-5">
                ...
                {import.meta.env.VITE_USE_MOCK_API !== 'false' && (
                  <Alert variant="secondary" className="auth-alert mock-hint" style={{ fontSize: '0.75rem' }}>
                    <strong>🧪 Dev mode — mock accounts:</strong>
                    <table style={{ width: '100%', marginTop: '0.35rem', borderCollapse: 'separate', rowGap: 2 }}>
                      <tbody>
                        {[
                          ['patient@medeasy.pk',    'patient123',    'Patient'],
                          ['doctor@medeasy.pk',     'doctor123',     'Doctor'],
                          ['pharmacist@medeasy.pk', 'pharmacist123', 'Pharmacist'],
                          ['admin@medeasy.pk',      'admin123',      'Admin'],
                        ].map(([email, pw, role]) => (
                          <tr key={role}>
                            <td style={{ color: '#374151', paddingRight: 6 }}>{role}</td>
                            <td><code style={{ fontSize: '0.72rem' }}>{email}</code></td>
                            <td><code style={{ fontSize: '0.72rem' }}>{pw}</code></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
